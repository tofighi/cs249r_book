...
* Discover software tools like frameworks and model conversion platforms that enable the deployment of optimized models
...
![A visualization of the training procedure of knowledge distillation. Note how the logits or soft labels of the teacher model are used to provide a distillation loss for the student model to learn from (@noauthor_knowledge_nodate).](images/png/modeloptimization_knowledge_distillation.png)
...
![Three floating-point formats (@threefloat).](https://storage.googleapis.com/gweb-cloudblog-publish/images/png/Three_floating-point_formats.max-624x261.png)
...
![In this visualization, we can note the massive reductions in energy use by quantized operations (@energyproblem).](images/png/efficientnumerics_100x.png)

![Tables comparing energy use by quantized operations (@energyproblem).](images/png/efficientnumerics_horowitz.png)

![Graph showing the speed differences for three different models in normal and quantized form.](images/png/efficientnumerics_int8vsfloat.png)
...
![Sine Wave](images/png/efficientnumerics_sinewave.png)
...
![Quantized Sine Wave](images/png/efficientnumerics_quantizedsine.png)
...
![Figure comparing the sizes of three models with their quantized forms](images/png/efficientnumerics_reducedmodelsize.png)
...
![Comparison between uniform quantization (left) and non-uniform quantization (right) (@surveyofquant).](images/png/efficientnumerics_uniformnonuniform.png)
...
![](images/png/efficientnumerics_binary.png)
...
![Histogram of input activations to layer 3 in ResNet50 and calibrated ranges (@intquantfordeepinf).](images/png/efficientnumerics_calibrationcopy.png)
...
![Illustration of symmetric quantization (left) and asymmetric quantization (right). Symmetric quantization maps real values to [-127, 127], and asymmetric maps to [-128, 127] (@surveyofquant).](images/png/efficientnumerics_symmetry.png)
...
2. Groupwise Quantization: This approach groups different channels inside a layer to calculate the clipping range. This method can be helpful when the distribution of parameters across a single convolution/activation varies a lot. In practice, this method was useful in Q-BERT [@DBLP:journals/corr/abs-1909-05840] for quantizing Transformer [@vaswani2023attention] models that consist of fully-connected attention layers. The downside with this approach comes with the extra cost of accounting for different scaling factors.
...
![Table showing the differences between different precisions of quantization.](images/png/efficientnumerics_lecturenote.png)

![In PTQ, a pretrained model is calibrated using calibration data (e.g., a small subset of training data) to compute the clipping ranges and scaling factors (@surveyofquant).](images/png/efficientnumerics_PTQ.png)
...
![In QAT, a pretrained model is quantized and then finetuned using training data to adjust parameters and recover accuracy degradation. Note: the calibration process is often conducted in parallel with the finetuning process for QAT (@surveyofquant).](images/png/efficientnumerics_QAT.png)

![Note that QAT is an extension of PTQ. It receives the model quantized by PTQ and retrains it to finetune quantized parameters (@deci).](images/png/efficientnumerics_PTQQAT.png)

![Summary of Post Training Quantization and Quantization Aware Training. PTQ reports the best accuracy and corresponding calibration for each model (@intquantfordeepinf).](images/png/efficientnumerics_PTQQATsummary.png)
...
![Diagram of quantizing weights and activations](images/png/efficientnumerics_weightsactivations.png)
...
![The model size of large language models is developing at a faster pace than the GPU memory in recent years, leading to a big gap between the supply and demand for memory. Quantization and model compression techniques can help bridge the gap (@smoothquant).](images/png/efficientnumerics_modelsizes.png)
...
![Benefits of lower precision data types for tensor operations on the NVIDIA Turing GPU architecture (@intquantfordeepinf).](images/png/efficientnumerics_benefitsofprecision.png)
...
![Accuracy v.s. compression rate under different compression methods. Pruning and quantization works best when combined (@deepcompress).](images/png/efficientnumerics_qp1.png)

![Pruning doesn't hurt quantization. Dashed: quantization on unpruned network. Solid: quantization on pruned network; Accuracy begins to drop at the same number of quantization bits whether or not the network has been pruned. ALthough pruning made the number of parameters less, quantization still works well, or even better (3 bits case on the left figure) as in the unpruned network (@deepcompress).](images/png/efficientnumerics_qp2.png)
...
![Throughput comparison of different commerical edge processors for NN inference at the edge (@surveyofquant).](images/png/efficientnumerics_edgequant.png)
...
![Taxonomy of HW-NAS [@ijcai2021p592]](images/png/modeloptimization_HW-NAS.png)
...
![A diagram showing how search spaces with high probability of finding an architecture with large number of FLOPs provide models with higher accuracy [@lin2020mcunet]](images/png/modeloptimization_TinyNAS.png)
...
![A figure showing how Computing in Memory can be used for always-on tasks to offload tasks of the power consuming processing unit [@zhou2021analognets]](images/png/modeloptimization_CiM.png)
...
![A Diagram showing the workflow with the hls4ml framework [@fahim2021hls4ml]](images/png/modeloptimization_hls4ml.png)
...
![A diagram showing how an FPGA was used to offload data preprocessing of the general purpose computation unit. [@app112211073]](images/png/modeloptimization_preprocessor.png)
...
![A chart showing a comparison between the performance of SplitNets vs all on sensor and all on aggregator approaches. [@dong2022splitnets]](images/png/modeloptimization_SplitNets.png)
...
![A figure showing the sparse columns of the filter matrix of a CNN that are aggregated to create a dense matrix that, leading to smaller dimensions in the matrix and more efficient computations [@kung2018packing]](images/png/modeloptimization_sparsity.png)
...
![A smooth histogram of quantization error. [@kuzmin2022fp8]](images/png/modeloptimization_quant_hist.png)
