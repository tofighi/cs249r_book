...
![Tables comparing energy use by quantized operations (@energyproblem).](images/png/efficientnumerics_horowitz.png){#fig-quantized-energy}
...
![In this visualization, we can note the massive reductions in energy use by quantized operations (@energyproblem).](images/png/efficientnumerics_100x.png){#fig-operations-energy-comparison}
...
![Comparison between uniform quantization (left) and non-uniform quantization (right) (@surveyofquant).](images/png/efficientnumerics_uniformnonuniform.png){#fig-quantization-uniformity}
...
![Histogram of input activations to layer 3 in ResNet50 and calibrated ranges (@intquantfordeepinf).](images/png/efficientnumerics_calibrationcopy.png){#fig-resnet-activations-histogram}
...
![Illustration of symmetric quantization (left) and asymmetric quantization (right). Symmetric quantization maps real values to [-127, 127], and asymmetric maps to [-128, 127] (@surveyofquant).](images/png/efficientnumerics_symmetry.png){#fig-quantization-symmetry}
...
![In PTQ, a pretrained model is calibrated using calibration data (e.g., a small subset of training data) to compute the clipping ranges and scaling factors (@surveyofquant).](images/png/efficientnumerics_PTQ.png){#fig-PTQ-diagram}
...
![In QAT, a pretrained model is quantized and then finetuned using training data to adjust parameters and recover accuracy degradation. Note: the calibration process is often conducted in parallel with the finetuning process for QAT (@surveyofquant).](images/png/efficientnumerics_QAT.png){#fig-QAT-diagram}

![Note that QAT is an extension of PTQ. It receives the model quantized by PTQ and retrains it to finetune quantized parameters (@deci).](images/png/efficientnumerics_PTQQAT.png){#fig-QAT-PTQ-relation}

![Summary of Post Training Quantization and Quantization Aware Training. PTQ reports the best accuracy and corresponding calibration for each model (@intquantfordeepinf).](images/png/efficientnumerics_PTQQATsummary.png){#fig-quantization-methods-summary}
...
![Benefits of lower precision data types for tensor operations on the NVIDIA Turing GPU architecture (@intquantfordeepinf).](images/png/efficientnumerics_benefitsofprecision.png){#fig-nvidia-turing}
...
![Accuracy v.s. compression rate under different compression methods. Pruning and quantization works best when combined (@deepcompress).](images/png/efficientnumerics_qp1.png){#fig-compression-methods}

![Pruning doesn't hurt quantization. Dashed: quantization on unpruned network. Solid: quantization on pruned network; Accuracy begins to drop at the same number of quantization bits whether or not the network has been pruned. ALthough pruning made the number of parameters less, quantization still works well, or even better (3 bits case on the left figure) as in the unpruned network (@deepcompress).](images/png/efficientnumerics_qp2.png){#fig-pruning-quantization-effect}
...
![Throughput comparison of different commerical edge processors for NN inference at the edge (@surveyofquant).](images/png/efficientnumerics_edgequant.png){#fig-throughout-comparison}
...
![Taxonomy of HW-NAS [@ijcai2021p592]](images/png/modeloptimization_HW-NAS.png){#fig-hw-nas}
...
![A diagram showing how an FPGA was used to offload data preprocessing of the general purpose computation unit. [@app112211073]](images/png/modeloptimization_preprocessor.png){#fig-fpga-preprocessing}
...
![Before/after diagram showing GraphOptimizer fusing operators in a sample graph](https://www.researchgate.net/publication/347955230/figure/fig5/AS:975063935365122@1609484766968/shows-an-example-of-how-a-graph-could-be-optimized-by-the-mapping-toolchain-In-the.png){#fig-graph-optimizer}
